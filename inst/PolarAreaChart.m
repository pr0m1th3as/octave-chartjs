  ...
  methods (Access = public)
    ...
    function this = PolarAreaChart (data, labels, varargin)
      ...
      if (! isnumeric (labels) && ! iscellstr (labels))
        error ("PolarAreaChart: labels can be either numeric or cellstring.");
      endif
      ...
      while (numel (varargin) > 0)
        switch (lower (varargin{1}))
          ...
          case "chartid"
            val = varargin{2};
            if (! ischar (val))
              error ("PolarAreaChart: 'ChartID' must be a character vector.");
            endif
            this.chartID = val;
          ...
          case "chartwidth"
            ...
            if (! isnumeric (val) || val < 0 || val > 100)
              error ("PolarAreaChart: 'ChartWidth' must be a scalar percentage.");
            endif
            this.chartID = val;
